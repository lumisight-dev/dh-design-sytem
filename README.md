# Datahouse React Design System Template
...
- [Datahouse React Design System Template](#datahouse-react-design-system-template)
...
```jsx
import {Button} from '@lumisight-dev/dh-react-design-system';
...
* Yarn (NPM is installed when you install Node.js on your machine)
  * npm install -g yarn
...
```bash
yarn add -D rollup @rollup/plugin-node-resolve @rollup/plugin-typescript @rollup/plugin-commonjs rollup-plugin-dts
```
...
```bash
yarn @YOUR_GITHUB_USERNAME/YOUR_REPOSITORY_NAME
```

So for my project for example its: `yarn add @lumisight-dev/dh-react-design-system`
...
And when we run `yarn start` again, there tucked up in the corner is our _Hello world!_ button.  
...
The other is to update some of our dependencies to `peerDependencies`.  With rollup's [peer dependencies](https://www.npmjs.com/package/rollup-plugin-peer-deps-external) plugin we can tell the projects that are using our libraries which dependencies are required (like React) but won't actually bundle a copy of React with the library itself.  If the consumer already has React in their project it will use that, otherwise it will get installed when they run `yarn`.

First we will install these two plugins:

```bash
yarn add -D rollup-plugin-peer-deps-external rollup-plugin-terser
```
...
Now we should be able to run our tests with `yarn test`... but... there is one more problem!
...
If you simply run a fresh `yarn install` command it will install all the `peerDependencies` of the libraries you are using.  Before running this you may need to delete your `package-lock.json` and `node_modules` directory.  They will be regenerated automatically after your fresh install.  
...
Now you will be able to run `yarn storybook` and see all your SCSS styles.
